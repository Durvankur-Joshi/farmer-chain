...
def test_04_fpo_farmer_workflow():
    print_header("Step 4: FPO-Farmer Bidding Workflow")
    fpo_headers = {"Authorization": f"Bearer {STATE['fpo_token']}"}
    farmer_headers = {"Authorization": f"Bearer {STATE['farmer_token']}"}

    quote_data = {"product_name": "Organic Wheat", "category": "Grains", "description": "High quality organic wheat", "quantity": "500.00", "unit": "kg", "deadline": "2025-12-31"}
    res = make_request("POST", "/fpo/quotes/", 201, "FPO Creates Quote", data=quote_data, headers=fpo_headers)
    STATE['fpo_quote_id'] = res['id']

    make_request("GET", "/farmer/quotes/open/", 200, "Farmer Views Open Quotes", headers=farmer_headers)

    bid_data = {"bid_amount": "20.50", "delivery_time_days": 15, "comments": "Can supply high quality wheat."}
    # THIS is the call that was failing. With the serializer fix, it will now pass.
    res = make_request("POST", f"/farmer/quotes/{STATE['fpo_quote_id']}/bids/", 201, "Farmer Places Bid", data=bid_data, headers=farmer_headers)
    STATE['farmer_bid_id'] = res['id']
    
    make_request("POST", f"/fpo/bids/farmer/{STATE['farmer_bid_id']}/accept/", 200, "FPO Accepts Farmer Bid", headers=fpo_headers)

def test_05_retailer_fpo_workflow():
    print_header("Step 5: Retailer-FPO Bidding Workflow")
    retailer_headers = {"Authorization": f"Bearer {STATE['retailer_token']}"}
    fpo_headers = {"Authorization": f"Bearer {STATE['fpo_token']}"}

    quote_data = {"product_name": "Packaged Wheat Flour", "category": "Processed Grains", "description": "10kg bags of flour", "quantity": "200.00", "unit": "bags", "deadline": "2025-12-31"}
    res = make_request("POST", "/retailer/quotes/", 201, "Retailer Creates Quote", data=quote_data, headers=retailer_headers)
    STATE['retailer_quote_id'] = res['id']

    make_request("GET", "/fpo/quotes/retailer/open/", 200, "FPO Views Retailer Quotes", headers=fpo_headers)

    bid_data = { "bid_amount": "150.00", "delivery_time_days": 10, "comments": "We can process and package." }
    # This call would also have failed without the fix.
    res = make_request("POST", f"/fpo/quotes/retailer/{STATE['retailer_quote_id']}/bids/", 201, "FPO Places Bid on Retailer Quote", data=bid_data, headers=fpo_headers)
    STATE['fpo_bid_id'] = res['id']

    make_request("POST", f"/retailer/bids/fpo/{STATE['fpo_bid_id']}/accept/", 200, "Retailer Accepts FPO Bid", headers=retailer_headers)

    payment_data = {"transaction_hash": f"0xTXHASH{generate_random_string(16).upper()}"}
    make_request("POST", f"/retailer/bids/fpo/{STATE['fpo_bid_id']}/confirm-payment/", 200, "Retailer Confirms Payment", data=payment_data, headers=retailer_headers)

def test_06_negotiation_workflow():
    print_header("Step 6: Negotiation Workflow")
    fpo_headers = {"Authorization": f"Bearer {STATE['fpo_token']}"}
    farmer_headers = {"Authorization": f"Bearer {STATE['farmer_token']}"}

    quote_data = {"product_name": "Organic Barley", "category": "Grains", "description": "For negotiation test", "quantity": "100.00", "unit": "kg", "deadline": "2025-11-30"}
    res = make_request("POST", "/fpo/quotes/", 201, "[NEGOTIATION] FPO Creates New Quote", data=quote_data, headers=fpo_headers)
    negotiation_quote_id = res['id']

    bid_data = {"bid_amount": "30.00", "delivery_time_days": 20, "comments": "This bid is for negotiation."}
    res = make_request("POST", f"/farmer/quotes/{negotiation_quote_id}/bids/", 201, "[NEGOTIATION] Farmer Places Bid", data=bid_data, headers=farmer_headers)
    STATE['negotiation_bid_id'] = res['id']
    
    negotiation_start_data = {"content_type": "farmer.farmerbid", "object_id": STATE['negotiation_bid_id']}
    res = make_request("POST", "/negotiation/start/", 201, "FPO Starts Negotiation", data=negotiation_start_data, headers=fpo_headers)
    STATE['negotiation_id'] = res['id']
    
    counter_offer_data = {"message": "I can do it for 28.00 in 18 days.", "counter_amount": "28.00", "counter_delivery_time_days": 18}
    make_request("POST", f"/negotiation/{STATE['negotiation_id']}/", 201, "Farmer Sends Counter-Offer", data=counter_offer_data, headers=farmer_headers)

    res = make_request("GET", f"/negotiation/{STATE['negotiation_id']}/", 200, "FPO Views Negotiation Details", headers=fpo_headers)
    
    last_message = res['messages'][-1]['message']
    if "28.00" in last_message:
        print(f"{Colors.OKGREEN}SUCCESS: Counter-offer message found in negotiation history.{Colors.ENDC}")
    else:
        print(f"{Colors.FAIL}FAILURE: Counter-offer message not found!{Colors.ENDC}")
        sys.exit(1)
...
def main():
    ...
    test_03_user_logins()
    test_04_fpo_farmer_workflow()
    test_05_retailer_fpo_workflow()
